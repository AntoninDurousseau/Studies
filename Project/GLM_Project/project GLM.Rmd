...
exam <- exam %>%
  mutate(
    ...
    trav_time = factor(trav_time, levels = c(1,2,3,4),labels = c("<15 min","15–30 min","30–60 min",">60 min"),ordered = TRUE),
...
```{r}
num_vars <- exam %>% dplyr::select(where(is.numeric))   #select quantitative variables

num_vars <- num_vars %>% dplyr::select(-y) #we remove the numerical variables
summary(num_vars)
...
for (var in names(num_vars)) { #For each quantitative variable, we plot a graph of the exam score as a function of the variable, adding the observations of the variables, the loses curve, and the associated linear curve.
...
```{r}
cat_vars <- exam %>% dplyr::select(where(is.factor)) #we select the categorical variables

cat_tables <- list()

for (var in names(cat_vars)) { #For each categorical variable, create a table in which you count the observations by category and the proportion of that category for the variable.
  
  tab <- table(cat_vars[[var]])
  prop <- round(100 * prop.table(tab), 1)
  
  cat_tables[[var]] <- data.frame(Category = names(tab), Count = as.vector(tab), Proportion = as.vector(prop))
}
...
for (var in names(cat_vars)) {
  #For each categorical variable, we will create a box plot with the exam score, display the observations, means and medians.
  p <- ggplot(exam,
              aes(x = .data[[var]],
                  y = y,
                  colour = .data[[var]],
...
```{r}
#For each interaction between two variables (one quantitative and one categorical) of interest expressed in the ADE, we visualize the exam graph as a function of the quantitative variable and stratify by the categorical variable.
...
```{r}
#For an interaction between two quantitative variables, we will categories one of them and do the same thing as before.
...
```{r}
#For an interaction between two categorical variables, we will calculate the averages and plot a graph based on them. We have also connected the points for better visualization.
...
```{r}
exam_top <- exam %>% dplyr::select(-agecat, -attend_pct) #Remove the attendance categorical and age categorical variables.

dim(exam_top) #we check the dimension
```
...
```{r}
mod0<- lm(y~1,data = train1) #model reduce to the intercept

modfull <- lm(y~ .,data = train1) #full model
```
...
modboth <- stepAIC(mod0,y~ attend_pct_cat + parent_educ+ study_hrs + sleep_qual +  trav_time + web_access + extra_act +sleep_hrs + school_type + study_method + age + sexe,data=train1,trace=T,direction=c('both'))  #model with AIC both direction
...
modbothBIC <- stepAIC(mod0,y~ attend_pct_cat + parent_educ+ study_hrs + sleep_qual +  trav_time + web_access + extra_act +sleep_hrs + school_type + study_method + age + sexe,data=train1,trace=F,k=log(n),direction=c('both'))
...
```{r}
modanova <- lm(y~ . -sexe,data = train1) #model derived from ANOVA

namemod = c("modboth","modanova") #vector of anova name
p_value1 = c(anova(modboth,modanova)$`Pr(>F)`[2],anova(modboth,modfull)$`Pr(>F)`[2]) #vector of p-values
data.frame(namemod,p_value1) #table containing the p-values of the ANOVA's
...
```{r}
m_sleep <- mean(train1$sleep_hrs, na.rm = TRUE) #average of the sleep hour variable

#we transform the variable in the train by centering it and adding the squared term
train <- train1 %>%
  mutate(
...
#we transform the variable in the test by centering it and adding the squared term
test <- test1 %>%
  mutate(
    sleep_c  = sleep_hrs - m_sleep,
    sleep_c2 = sleep_c^2
...
#we repeat for the variable study hours
...
p_value1 = c(anova(mod1,mod1bis)$`Pr(>F)`[2],anova(mod_1,mod_1bis)$`Pr(>F)`[2]) #vector of p-value
data.frame(namemod,p_value1) #table of p value of anova test
...
R = c(summary(mod1)$adj.r.squared,summary(mod1bis)$adj.r.squared,summary(mod_1)$adj.r.squared,summary(mod_1bis)$adj.r.squared) #vector of criterion

#calcul of CP criterion
n1 <- nrow(train)
p1 <- length(coef(mod1))
...
```{r}
mod00 <- lm(y~(sleep_c + sleep_c2) + attend_pct_cat + (study_c +study_c2) + trav_time + sleep_qual + study_method +extra_act+ parent_educ*school_type + web_access, data = train) #model to test the interaction parent education X school type
```

### simple interations with study hours
...
```{r}
#after comparison, we keep models 5 and 7 (because much less interaction than model 9 -> over fitting) 

modI <- mod5
modII <- mod7
```

### interaction with sleep quality
...
### interaction with attendance cat
...
```{r}
#we conserve the model 29
modVI <- mod29
```

## We select the best model from those selected and those build with other interaction (triple interactions).
...
```{r}
#list of selected models with the same names
...
#we order for a better visualization smaller to taller
aic[order(aic$AIC), ]
bic[order(bic$BIC), ]
```
...
```{r}
#R2 adjusted criteria table
adj_r2 <- data.frame(
  Model = names(models),
  Adj_R2 = sapply(models, function(m) summary(m)$adj.r.squared)
)

#ordered for a better understanding taller to smaller
...
```{r}
#full model in suitability with our 12 selected models
full_mod <- lm(
...
#ordered for a better understanding smaller to taller
...
# due to this table we pre select modI, modVII and modVIII 
```
...
```{r}
library(ggfortify)
autoplot(modVIII,1,colour = 'skyblue') #residuals vs fitted
autoplot(modVIII,2,colour = 'tan1') #normal QQ plot
autoplot(modVIII,3,colour = 'skyblue') #scale location
autoplot(modVIII,4,colour = 'tan1') #cook's distance
```
...
```{r}
cook <- cooks.distance(modVIII) #vector of cook's distance

# Top 3 most influential observations
head(sort(cook, decreasing = TRUE), 3) #we choose the top 3 higher distance
```

```{r}
idx <- order(cook, decreasing = TRUE)[1:3] #we conserve the nuber of the observation

modVIII_reduced <- lm(formula(modVIII),data = train[-idx, ]) #we remove the 3 most influential observations of the model


#we compare the summary to seen if there is aberrant coefficients
summary(modVIII)
...
```{r}
pred_train <- predict(modVIII, newdata = train) #vector of value predicted on the train
pred_test  <- predict(modVIII, newdata = test) #vector of value predicted on the test


#naive base model
base_hat_train <- rep(mean(train$y), nrow(train)) #we calculate the mean of y in train target then we repeat the number of observation in train
base_hat_test  <- rep(mean(train$y), nrow(test)) #same for test

df_train <- data.frame(y = train$y, yhat = pred_train, split = "Train") 
df_test  <- data.frame(y = test$y,  yhat = pred_test,  split = "Test") 
df_all   <- rbind(df_train, df_test) #table with y observed, y hat predicted and split to see if the observation is in train or test
```

```{r}
ggplot(df_all, aes(x = yhat, y = y)) + 
  geom_point(alpha = 0.35, col = 'skyblue') + 
  geom_smooth(method = "loess", se = FALSE,col = 'tan1') + 
  geom_abline(slope = 1, intercept = 0, linetype = 2, col='red') + 
  facet_wrap(~split) +labs(title = "Calibration: Observed vs Predicted (modVIII)", x = "Predicted score", y = "Observed score")
```

## Mean observed score on test data set.

```{r}
#Separately, we divide that into deciles, then group them by decile and by train and test category, then summaries each group as an average, and finally obtain a table with 10 points per split.
...
#we separate the criterion for a better understanding of the following plot
...
names(coef_tab) <- c("Estimate", "Std_Error", "t_value", "p_value") # Coefficients + SE

# CI 95%
ci <- confint(modVIII, level = 0.95)   

#we separate the low and high limit of the CI
coef_tab$CI_low  <- ci[, 1]
coef_tab$CI_high <- ci[, 2]

coef_tab_final <- coef_tab[, c("Estimate", "Std_Error", "CI_low", "CI_high")] #keep important one
...
# Summary (mean + standard deviation) by sample size
summary_res <- res %>%
  group_by(frac) %>%
  summarise(
...
```{r}
# Choice of scenarios in centered units
A_study_c <- -1.0
B_study_c <-  1.5
A_sleep_c <- -1.0
B_sleep_c <-  1.0

#we generate 2 new profiles
new_profiles <- data.frame(
  
  sleep_c  = c(A_sleep_c,  B_sleep_c), sleep_c2 = c(A_sleep_c^2, B_sleep_c^2),
  
  study_c  = c(A_study_c,  B_study_c),study_c2 = c(A_study_c^2, B_study_c^2),
  
  attend_pct_cat = factor(c("62-72", "81-100"), levels =levels(train$attend_pct_cat)),
  
  trav_time = factor(c(levels(train$trav_time)[length(levels(train$trav_time))],levels(train$trav_time)[1]),levels = levels(train$trav_time)),
  
  sleep_qual = factor(c(levels(train$sleep_qual)[2],levels(train$sleep_qual)[length(levels(train$sleep_qual))]),levels = levels(train$sleep_qual)),
...
pred_ci <- predict(modVIII, newdata = new_profiles, interval = "confidence", level = 0.95) #CI 95%
pred_out <- cbind(Profile = rownames(new_profiles), as.data.frame(pred_ci))
pred_out
```

```{r}
TermsX <- delete.response(terms(modVIII)) #remove the y of this new observation
